...
def showdatasgao(datingDataMat, datingLabels):
	#设置汉字格式
	#font = FontProperties(fname=r"c:\windows\fonts\simsun.ttc", size=14)
	font=FontProperties(fname="/System/Library/Fonts/STHeiti Light.ttc",size=14)
	#将fig画布分隔成1行1列,不共享x轴和y轴,fig画布的大小为(13,8)
	...
	for i in datingLabels:
		...
		if i == 3:
			LabelsColors.append('red')
	ax.scatter(datingDataMat[:,0],datingDataMat[:,1],color=LabelsColors,s=10.0,alpha=1)
	...
	plt.setp(ax_title_text, size=9, weight='bold', color='red')  
...
#	datingClassTest()
	...
	print(datingDataMat)
	print(datingLabels)
	showdatasgao(datingDataMat, datingLabels)